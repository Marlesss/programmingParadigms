...
(defn sameForm [first & others] {:pre [(or (all number? (conj others first)) (all vector? (conj others first)))] :post [(true? %)]}
  (letfn [(binSameForm [s1 s2] (or (and (number? s1) (number? s2)) (and (vector? s1) (vector? s2) (== (count s1) (count s2)) (all true? (mapv binSameForm s1 s2)))))]
    (all (partial binSameForm first) others)))
(defn isMatrix [matr] (and (vector? matr) (all vector? matr) (apply == (mapv count matr))))
(defn forAllNumbers [f elem] {:pre [(or (number? elem) (vector? elem))] :post [(sameForm elem %)]} (if (number? elem) (f elem) (mapv (partial forAllNumbers f) elem)))
...
(defn m*m [& matrs]
  ...
  (letfn [(binm*m [m1 m2]
            {:pre  [(isMatrix m1) (isMatrix m2) (== (count (first m1)) (count m2))]
    ...
    (reduce binm*m matrs)))
(defn m*v [matr & vecs]
  {:pre  [(isMatrix matr) (all vector? vecs)]
   :post [(vector? %) (== (count %) (count matr))]}
  (reduce #(mapv (fn [st] (reduce + (v* st %2))) %1) matr vecs))
(defn transpose [matr]
  {:pre  [(isMatrix matr)]
   :post [(isMatrix %) (== (count %) (count (first matr))) (== (count (first %)) (count matr))]}
